...
interface RestaurantIconProps {
  type: 'local' | 'rice' | 'pizza' | 'burger' | 'grill' | 'cafe';
  size?: 'sm' | 'md' | 'lg' | 'xl';
  className?: string;
}

const RestaurantIcon: React.FC<RestaurantIconProps> = ({ type, size = 'md', className = '' }) => {
  const sizeClasses = {
    sm: 'w-12 h-12',
    md: 'w-16 h-16',
    lg: 'w-20 h-20',
  ...
  const iconColors = {
    ...
    grill: 'bg-rose-100 text-rose-700',
    cafe: 'bg-brown-100 text-brown-700'
  };

  const icons = {
    local: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-2/3 h-2/3">
  ...
  return (
    <div className={`${sizeClasses[size]} ${iconColors[type]} rounded-2xl flex items-center justify-center flex-shrink-0 ${className}`}>
      {icons[type]}
    </div>
  );
};
